@extends('web.layouts.app')
@section('content')
<div class="container-fluid">
   <div class="row align-items-center pt-2">
      <div class="col-lg-3 col-sm-3">
         <a href="/" class="hvr-icon-back text-uppercase font-13 fw-bold text-purple">
         <i class="fa-regular fa-circle-left hvr-icon"></i>
         BACK TO HOME
         </a>
      </div>
      <div class="col-lg-6 col-sm-6 text-center">
         <h1 class="font-36 fw-bold text-uppercase text-purple">PENDING DISPACHED ORDERS


         </h1>
         ITEM SHOP
      </div>
      <div class="col-lg-3 col-sm-3 pt-lg-0 pt-3 text-end">
         
         <p  class="w-auto text-center text-uppercase font-12
            text-white rounded-pill py-2 px-3 bg-secondary fw-500 align-items-center
            "> PENDING DISPATCHED ORDERS
            <span class="badge badge-custom bg-white  text-dark ms-1 font-13">6</span>
         </p>
      </div>
   </div>
</div>
<div class="container-fluid py-lg-5 py-3 px-lg-5 ">
   <div class="row middle-hight">
      <div class="col-12">
         <div class="table-responsive  ">
            <table id="pendingDisOrder" class="table table-striped table-hover table-bordered">
               <thead class="text-white gradient-background text-uppercase fw-light font-14">
                  <th> 
                    DATE</th>
                  
                  <th>NAME
                  </th>
                  <th>CUSTO
                    MER
                    TYPE                
                  </th>
                  <th>PACKED BY 
                  </th>
                  <th>ORDER DETAILS
                  </th>
                  <th>ORDER TYPE
                  </th>

                  <th> STATUS</th>
                  </tr>
               </thead>
               <tbody class="font-13 fw-500 align-items-center">
                  <tr>
                     <td>01.01.2023</td>
                    
                     <td>PRAMOD THILINA
                        <a class="btn btn-info fw-500 font-11 px-2 w-100 rounded-pill text-white"
                           href="">  DETAILS</a>
                     </td>
                     <td>OFFICE
                        MEMBER</td>
                     <td>VIRANGA<br>
                        2025/12/29
                     </td>
                     <td> G6 EM SCIENCE TT BOOK -
                        1
                        </td>
                     <td>Delivery/Pick
                        Up
                        </td>
                      
                     <td>
                        <span class="btn btn-success font-11 px-3 fw-bolder rounded-pill  text-uppercase mb-2">DISPATCH</span>
                        <span class="btn btn-danger font-11 px-3 fw-bolder rounded-pill  text-uppercase mb-2">CANCEL</span>
                        
  

                      


                      
                     </td>
                  </tr>
               </tbody>
            </table>
         </div>
      </div>
   </div>
</div>
@endsection